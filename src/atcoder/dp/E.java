...
//        err.println("Execution Time : +" + (endTime - startTime) / 1000000 + " ms");
    ...
    static long solveDp(int maxPossibleVal, int itemNo, int w[],
                       int val[], int n) {
        // Base cases
        if (maxPossibleVal <= 0)
            return 0;

        //Return because not able to attain maxPossibleVal
        if (itemNo == n)
            return W_MAX;

        // Already processed
        if (v[maxPossibleVal][itemNo])
            return dp[maxPossibleVal][itemNo];

        v[maxPossibleVal][itemNo] = true;

        // Recurrence relation
        // min(don't include this item, include this item)
        dp[maxPossibleVal][itemNo] = Math.min(solveDp(maxPossibleVal, itemNo + 1, w, val, n),
                (long)w[itemNo] + solveDp(maxPossibleVal - val[itemNo],
                        itemNo + 1, w, val, n));
    ...
    static int maxWeight(int w[], int val[], int n, int c) {
        for (int maxPossibleVal = V_SUM_MAX; maxPossibleVal >= 0; maxPossibleVal--) {
            if (solveDp(maxPossibleVal, 0, w, val, n) <= c) {
